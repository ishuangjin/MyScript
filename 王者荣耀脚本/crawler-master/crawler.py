...
'''数据库配置'''
db_cfg = {'host': '106.55.231.58',
          'port': '3306',
          'database': 'study_crawler_77',
          'user': 'root',
          'password': 'Jiny1216'}
...
origin_user = {
    'open_id': 'owanlsnSqiwNnd72Y3KpZpxxcb4c',
    'zone_area_id': 4037
    # 'open_id': 'owanlskn3-DCpZnhRUWgApywOZd0',
    # 'open_id': 'owanlsm3Ras-T4URhuslOy_4lD5s',  # 改名卡太多吖
    # 'zone_area_id': 3089  # 改名卡太多吖
    # 'zone_area_id': 4037
}

fetch = Fetcher(
    key='',
    zone_area_id=origin_user['zone_area_id'],
    openid=origin_user['open_id'],
    ticket='iv%2Fa%2BS5sRzuK4c5tNlEL4ltt9crcltilcW8N3q1kTI3IGaehVdDJaeUNvfnfY8F%2F')
...
def init_user():
    ...
    if res is not None and res['errcode'] is not None and res['errcode'] == 0:
        u = res['user_info']
        if len(db.query("select id from user where open_id='%s' and zone_area_id=%s" % (
                u.get('open_id'), u.get('zone_area_id')))) == 0:
            ...
            user_dic = (
                u.get('open_id'), u.get('zone_area_id'), u.get('nick_name'), u.get('head_img_url'), u.get('game_name'),
            ...
            user_dic = (u.get('nick_name'), u.get('head_img_url'), u.get('game_name'),
                        u.get('service_name'), u.get('rank_desc'), u.get('winning_percentage'), u.get('rank_lift'),
                        u.get('win_desc'), u.get('rank_star'), u.get('ladder_score'), u.get('open_id'),
                        u.get('zone_area_id')
                        )
...
def init_user_hero_info():
    ...
    if res is not None and res['errcode'] is not None and res['errcode'] == 0:
        ...
        data_list = [
            (hero['hero_id'], fetch.get_working_open_id(), fetch.get_working_zone_area_id(), hero.get('hero_name'),
...
def init_battle_info_list_all():
    ...
    while not the_end:
        ...
        if res is not None and res['errcode'] is not None and res['errcode'] == 0:
            ...
            sql = "INSERT IGNORE INTO `battle` (`game_seq`, `game_type`, `game_svr_entity`, `relay_svr_entity`, `dt_event_time`) VALUES (%s, %s, %s, %s, %s);"
            data_list = [
                (battle['game_seq'], battle['game_type'], battle['game_svr_entity'], battle['relay_svr_entity'],
                 battle['dt_event_time']) for battle in battle_list]
            db.executemany(sql, data_list)

            for battle in battle_list:
                db_battle_list = get_battle_info(battle['game_seq'], battle['game_svr_entity'],
                                                 battle['relay_svr_entity'])
                if db_battle_list and len(db_battle_list) > 0:
                    logger.info("数据库已存在对战详情: %s, %s, %s" % (
                        battle['game_seq'], battle['game_svr_entity'], battle['relay_svr_entity']))
                    continue
                try:
                    user_list += init_battle_detail(battle['game_seq'], battle['game_svr_entity'],
                                                    battle['relay_svr_entity'])
                    time.sleep(1)
...
def init_battle_detail(game_seq, game_svr_entity, relay_svr_entity):
    ...
    if res is not None and res['errcode'] is not None and res['errcode'] == 0:
        ...
        data_list = [(battle_detail['blue_kill_cnt'], battle_detail['red_kill_cnt'], battle_detail['is_blue'],
                      battle_detail['game_time'], battle_detail['is_victory'], game_seq, game_svr_entity,
                      relay_svr_entity)]
        db.executemany(sql, data_list)
        ...
        detail_list = [(
            d.get('open_id'), d.get('name'), d.get('game_name'), d.get('hero_id'), game_seq, game_svr_entity,
            relay_svr_entity,
            battle_detail.get('game_type'), d.get('hero_name'), d.get('total_hurt'),
            d.get('total_hurt_hero'), d.get('suffer_hurt'),
            d.get('total_hurt_percent'), d.get('total_hurt_hero_percent'),
            d.get('suffer_hurt_percent'), d.get('kill_cnt'), d.get('dead_cnt'), d.get('assist_cnt'),
            d.get('is_blue'), d.get('grade_of_rank'), d.get('lose_mvp'),
            d.get('is_mvp'), d.get('pvp_level'), d.get('total_in_battle_coin'), d.get('used_time'),
            d.get('mvp_score_tth'), d.get('zone_area_id'), d.get('is_low_score'),
            d.get('is_victory'), d.get('acnt_camp'), d.get('is_five_army'), d.get('game_result'),
            d.get('six_kill'), d.get('seven_kill'), d.get('eight_kill'),
            d.get('game_score'), d.get('multi_camp_rank'), d.get('is_ai_pvp')) for d in
            user_battle_detail_list]
